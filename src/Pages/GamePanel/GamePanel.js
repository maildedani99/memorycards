...
const GamePanel = ({ themeColor }) => {
  const { animals, roatedCards, setRotatedCards } = UseGamePanel();
  ...
  return (
    <div className=" w-4/6 h-5/6 flex flex-row flex-wrap justify-around ">
      {array.map((item) => (
        <div className="w-1/6  mt-8" key={Math.random()}>
          <div className="" style={{ width: "100px", height: "150px" }}>
            <Card matched={matched} item={item} themeColor={themeColor} setRotatedCards={setRotatedCards} roatedCards={roatedCards} />
          </div>
        </div>
      ))}
    </div>
